...
export const ExploreView: React.FC<ExploreViewProps> = ({ 
  ...
  }, [currentConversation.length, scrollToBottom]);
  const handleSearch = useCallback(async (query: string) => {
    try {
      ...
      if (currentConversation.length > 0) {
        setConversations(prev => [...prev, currentConversation]); // Corrected
      }
      ...
      setCurrentConversation([newUserMessage, newAiMessage]);
  ...
  }, [gptService, onError, userContext, currentConversation]);
  ...
  const handleRelatedQueryClick = useCallback((query: string) => {
    ...
    // Scroll to bottom after searching
    setTimeout(() => {
      scrollToBottom();
    }, 100); // Delay ensures the new content is added before scrolling
        ...
        <div ref={messagesContainerRef} className="relative flex flex-col w-full overflow-y-auto">
          <div className="space-y-2 pb-16">
            ...
            {conversations.map((conversation, convIndex) => (
              <div key={convIndex} className="space-y-2">
                {conversation.map((message, msgIndex) => (
                  <div key={msgIndex} className="px-2 sm:px-4 w-full mx-auto">
            ...
            {currentConversation.map((message, index) => (
              <div key={index} className="px-2 sm:px-4 w-full mx-auto pt-8">
                <div className="max-w-3xl mx-auto">
                  {message.type === "user" ? (
                    <div className="w-full">
                      <div className="flex-1 text-base sm:text-lg font-semibold text-gray-100">
